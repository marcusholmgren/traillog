...
export default function AddWaypoint() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          setError(`Error getting location: ${err.message}. Please ensure location services are enabled and permission is granted.`);
        }
      );
    } else {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (latitude === null || longitude === null) {
      setError('Coordinates are required. Please ensure location services are enabled.');
      return;
    }
  ...
  return (
    ...
    >
      {/* Main content area */}
      <div className="flex flex-col flex-grow"> {/* Added flex-grow to allow "mt-auto" for buttons to work */}
        ...
        <div className="flex max-w-[480px] flex-wrap items-end gap-4 px-4 py-3">
          <label className="flex flex-col min-w-40 flex-1">
            ...
            <input
              ...
              value={latitude !== null && longitude !== null ? `${latitude.toFixed(6)}, ${longitude.toFixed(6)}` : 'Loading coordinates...'} // Formatted coordinates
              readOnly
            />
          </label>
        </div>
        ...
        {successMessage && <p className="text-green-500 px-4 py-2">{successMessage}</p>}
          ...
          {/* Image capture UI - keeping existing structure if needed, otherwise can be removed or adapted */}
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuCMiSQXTieU_81XZ3U3RaVUEh378ufxMCYTi_mn1_n2-EamIc7BhKyglQq4BOJ_w1IULjn9uU31oDroRyECb70bB6XsrrES9MuXzgxSUGF4YIVFJmiBlJlcGPThIC2dt9-o_Ab85vd4Kdd0FXecHvScSV9_aMDSdD0nJrsAcoZvfPmrYpIbmDmsDu_kviUkVWNTqXe7GS6mxYgzaguBxOI8VOBQ8jwFsL8Jj4WIhnnZ6FoHFWGmp9TkYWGRnZA4LCXTPj5bGCOWY-8')",
                }}
              ></div>
            </div>
          </div>
      ...
      <div className="mt-auto"> {/* This will push the buttons to the bottom because parent is flex-col and flex-grow */}
