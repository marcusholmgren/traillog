...
const mockRoutesData: db.Route[] = [
  { id: 1, name: "Scenic Drive", waypointIds: [1, 2, 3], createdAt: Date.now() - 10000 },
  { id: 2, name: "City Tour", waypointIds: [4, 5], createdAt: Date.now() - 5000 },
  { id: 3, name: "Mountain Hike", waypointIds: [6, 7, 8, 9], createdAt: Date.now() },
];

const mockWaypointsForView: Record<number, db.Waypoint> = {
    1: { id: 1, name: "Start Scenic", latitude: 10, longitude: 10, createdAt: Date.now()},
    2: { id: 2, name: "Mid Scenic", latitude: 11, longitude: 11, createdAt: Date.now()},
    3: { id: 3, name: "End Scenic", latitude: 12, longitude: 12, createdAt: Date.now()},
};
...
const renderWithRouter = (ui: React.ReactElement, { route = "/" } = {}) => {
  ...
  return render(
    <MemoryRouter initialEntries={[route]}>
      <Routes>
        <Route path="/saved_routes" element={ui} />
        <Route path="/create_route" element={<div>Create Route Page</div>} />
        <Route path="/map" element={<div>Map Page</div>} /> {/* Mock map page */}
      </Routes>
    </MemoryRouter>
  );
};
...
describe("SavedRoutesPage", () => {
  beforeEach(() => {
    ...
    (db.getWaypointById as vi.Mock).mockImplementation(async (id: number) => mockWaypointsForView[id]);
  ...
  it("loads and displays saved routes", async () => {
    renderWithRouter(<SavedRoutesPage />, { route: "/saved_routes" });
  ...
  it("shows an error message if loading routes fails", async () => {
    (db.getSavedRoutes as vi.Mock).mockRejectedValueOnce(new Error("Failed to fetch routes"));
    renderWithRouter(<SavedRoutesPage />, { route: "/saved_routes" });
    await waitFor(() => {
      expect(screen.getByText("Failed to load saved routes. Please try again.")).toBeInTheDocument();
    });
  });
  ...
  it("shows 'No Saved Routes Yet' message and 'Create New Route' button if no routes are present", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("It looks like you haven't saved any routes. Create one now!")).toBeInTheDocument();
    });
    const createButton = screen.getByRole('button', { name: /Create New Route/i });
    expect(createButton).toBeInTheDocument();
    fireEvent.click(createButton);
    expect(mockNavigateFn).toHaveBeenCalledWith("/create_route");
  });
  ...
  it("opens delete confirmation dialog when delete button is clicked", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Are you sure you want to delete the route "Scenic Drive"?/)).toBeInTheDocument();
    });
  });
  ...
  it("deletes a route and removes it from the list when confirmed", async () => {
    ...
    const deleteButtons = screen.getAllByLabelText(`Delete route ${routeToDelete.name}`);
  ...
  it("shows error if deleting a route fails", async () => {
    (db.deleteRoute as vi.Mock).mockRejectedValueOnce(new Error("Deletion failed"));
    renderWithRouter(<SavedRoutesPage />, { route: "/saved_routes" });
    await waitFor(() => screen.getByText("Scenic Drive"));

    const routeToDelete = mockRoutesData[0];
    fireEvent.click(screen.getAllByLabelText(`Delete route ${routeToDelete.name}`)[0]);
    await waitFor(() => screen.getByRole("dialog"));
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));

    await waitFor(() => {
      expect(screen.getByText(`Failed to delete route "${routeToDelete.name}". Please try again.`)).toBeInTheDocument();
    });
     // Route should still be in the list
    expect(screen.getByText(routeToDelete.name)).toBeInTheDocument();
    ...
    // For now, assume it's a global error message, so dialog would close.
     expect(screen.queryByRole("dialog")).not.toBeInTheDocument(); // Or check if it's still open depending on UX for error
  });
  ...
  it("navigates to map page with correct parameters when 'View' button is clicked", async () => {
    ...
    const viewButton = screen.getAllByLabelText(`View route ${routeToView.name}`)[0];
    ...
    await waitFor(() => {
      ...
      expect(mockNavigateFn).toHaveBeenCalledWith(`/map?waypoints=${expectedCoordinates}`);
    });
  });
  ...
  it("shows alert if trying to view a route with less than 2 valid waypoints", async () => {
    ...
    (db.getWaypointById as vi.Mock).mockImplementation(async (id: number) => {
        if (id === mockRoutesData[0].waypointIds[0]) return mockWaypointsForView[id];
    ...
    fireEvent.click(screen.getAllByLabelText(`View route ${routeToView.name}`)[0]);

    await waitFor(() => {
        expect(window.alert).toHaveBeenCalledWith("Route requires at least 2 valid waypoints to display.");
        expect(mockNavigateFn).not.toHaveBeenCalledWith(expect.stringContaining("/map"));
    });
  });
